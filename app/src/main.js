    ...
    transplantMatch: async function() {
        ...
        for (let i=0; i<donorCount; i++) {
            await this.contractInstance.methods.getDonor(donorIDs[i]).call().then(function(result){
                ...
                donorObj = { ID: donorIDs[i], name: result[0], bloodtype: result[3], organs: organsArr, organcount: organsArr.length,weight: result[5],height: result[6]};
                donor[i] = donorObj;
            });
        }
        console.log(donor);
        ...
        for (var i=0; i<patientCount; i++) {
            ...
            await this.contractInstance.methods.getPatient(patientIDs[i]).call().then(function(result){
                ...
                patientweight = result[5];
                patientheight= result[6];
            });
            ...
            for (var poi=0; poi < patientorgans.length; poi++) {
                ...
                for (var j=0; j<donorCount; j++) {
                    ...
                    for (let doi=0; doi < donor[j].organcount; doi++) {
                        ...
                        let weightDiff = Math.abs(patientweight - donor[j].weight);
                        let heightDiff = Math.abs(patientheight - donor[j].height);

                        if (weightDiff > 20 || heightDiff > 15) {  
                        console.log("Skipping donor due to weight/height mismatch.");
                        continue; // Skip this donor for this organ
                    }   
                        if (patientbloodtype==donor[j].bloodtype && patientorgans[poi]==donor[j].organs[doi]) {
                            ...
                            match = [
                                { "Patient Name": patientname, "Patient Organ": patientorgans[poi], "Patient Medical ID": patientIDs[i],"": "↔️", "Donor Medical ID": donorIDs[j], "Donor Organ": donor[j].organs[doi], "Donor Name": donor[j].name,"Weight Difference": weightDiff + " kg",
                                "Height Difference": heightDiff + " cm"},
                            ];
